test_that("extract_base_name strips UID suffix", {
  testthat::expect_equal(extract_base_name("exp_1_of_3.CSV"), "exp")
...
test_that("extract_base_name does not match lowercase extension", {
  testthat::expect_equal(extract_base_name("exp_1_of_3.csv"), "exp_1_of_3.csv")
})
...
test_that("find_raw_export_files only returns CSV files", {
  ...
  testthat::expect_equal(sort(basename(csv_files)), sort(c("a.CSV", "c.CSV")))
  testthat::expect_false(any(grepl("b.csv", csv_files, fixed = TRUE)))
})
