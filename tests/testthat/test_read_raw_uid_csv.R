test_that("read_raw_uid_csv parses datetime and returns columns", {
  ...
  testthat::expect_s3_class(df$datetime, "POSIXct")
  testthat::expect_equal(df$rfid[1], "ABC123")
})
...
test_that("read_raw_uid_csv errors on missing columns", {
  ...
  testthat::expect_error(read_raw_uid_csv(tmp), "Missing required")
})
...
test_that("read_raw_uid_csv errors on bad datetime", {
  ...
  testthat::expect_error(read_raw_uid_csv(tmp), "Failed to parse datetime")
})
